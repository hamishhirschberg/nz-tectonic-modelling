...
c
     ...
     2      xy(0:maxxy,maxy),xz(0:maxxz,maxz)
      real*8 s0,s,s2,alpha,s1,beta,sy0,si,alphaf,ext
...
c      if (iter.eq.nx2) goto 20
      if ((iter.eq.nx2*1.5d0).or.(s1.lt.s0*1.0d-48)) goto 20
      beta=s1/s
      do i=ix1,nx
            p(i)=ra(i)+beta*p(i)
      end do
...
c
      alpha=dsqrt(dfloat(nx2)/sy0)*alphaf
      do i=1,ny0
            y(i)=alpha*y(i)
            jmax=x0maxy(i)
            do j=0,jmax
...
c      if (iter.eq.nx2) goto 40
      if ((iter.eq.nx2*ext).or.(s1.lt.s0*1.0d-48)) goto 40
      beta=s1/s
      do i=ix1,nx
            p(i)=ra(i)+beta*p(i)
      end do
