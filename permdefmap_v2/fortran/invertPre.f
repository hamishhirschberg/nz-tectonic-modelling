...
c~       alpha=dsqrt(dfloat(nx2)/sy0)
c~       do i=1,ny0
c~             y(i)=alpha*y(i)
c~             jmax=x0maxy(i)
c~             do j=0,jmax
c~                   ay(j,i)=alpha*ay(j,i)
c~             end do
c~       end do
c~       if (ny1.eq.0) then
c~             do i=1,nx
...
c~             goto 50
c~       end if
c~       call scaler(nx,ny,nz,nzsq,zmin,csqlen,csq,nx1,maxxy,maxxz,
c~      1      x2maxy,x1maxz,x2maxz,xy,xz,ay,cz,p,w1,w2,q,asqinv)
c~       call getx2a(nx,ny,nz,nzsq,zmin,csqlen,csq,nx1,maxxy,maxxz,
c~      1      x2maxy,x1maxz,x2maxz,xy,xz,ay,cz,y,w1,w2,q)
c~       iter=0
c~       s0=0.0d0
c~       do i=ix1,nx
c~             x2(i)=0.0d0
...
c~ c
c~ 30    iter=iter+1
c~       call nextx2(nx,ny,nz,nzsq,zmin,csqlen,csq,nx1,maxxy,maxxz,
c~      1      x2maxy,x1maxz,x2maxz,xy,xz,ay,cz,p,w1,w2,q)
c~       s2=0.0d0
c~       do i=ix1,nx
c~             s2=s2+p(i)*q(i)
c~       end do
...
c~       s1=0.0d0
c~       do i=ix1,nx
c~             x2(i)=x2(i)+alpha*p(i)
c~             r(i)=r(i)-alpha*q(i)
c~             ra(i)=asqinv(i)*r(i)
c~             s1=s1+r(i)*ra(i)
c~       end do
c~       write(*,*) 'iter,s1=',iter,s1
c~ c      if (iter.eq.nx2) goto 40
c~       if ((iter.eq.nx2).or.(s1.lt.s0*1.0d-48)) goto 40
c~       beta=s1/s
c~       do i=ix1,nx
c~             p(i)=ra(i)+beta*p(i)
c~       end do
...
c~       goto 30
c~ c
c~ 40    call x2tox(nx,nz,nzsq,zmin,csqlen,csq,nx1,maxxz,
c~      1      x1maxz,x2maxz,xz,cz,x2,w2,x1)
c~       call xtoy(nx,ny,maxxy,x2maxy,xy,ay,x1,y1)
c~       do i=1,nx
c~             x1(i)=x0(i)+x1(i)
c~       end do
c~       do i=1,ny
...
c~             end do
c~       end if
c~       write(*,*) 'In the aposteriori solution the total sum of'
c~       write(*,*) ' squares for the scaled dynamics part is',sy0
c~       write(*,*) 'In the aposteriori solution the residual sum of'
c~       write(*,*) ' squares for the scaled dynamics part increases'
c~       write(*,*) ' through inclusion of observables to',s0
c~       write(*,*) 'The number of actual observables is',ny1
c~       write(*,*) 'In the aposteriori solution the residual sum of'
c~       write(*,*) ' squares for the actual observables is',s1
c~       write(*,*) 'So the total residual sum of squares reduces'
c~       write(*,*) ' through inclusion of observables to',s0+s1
c~ c
      write(2) sy0,s0,s1
c
      do i=1,nx
